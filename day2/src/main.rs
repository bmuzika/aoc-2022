...
fn main() {
    ...
    let input: Vec<(char, char)> = file
        ...
        .filter(|s| !s.is_empty())
        .map(|s| {
            let s: Vec<char> = s.split(' ').map(|s| s.chars().collect::<Vec<char>>()[0]).collect();
            (s[0], s[1])
        })
        .collect();
...
fn do_day2a(games: &[(char, char)]) -> u64 {
    games.iter().map(|s| (char_to_move(s.0), char_to_move(s.1))).fold(0, |acc, game| {
...
fn do_day2b(games: &[(char, char)]) -> u64 {
    games.iter().map(|s| (char_to_move(s.0), char_to_result(s.1))).fold(0, |acc, game| {
